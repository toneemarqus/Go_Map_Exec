...
func checkSSH(username, password, host string) bool {
	...
	cmd := exec.Command("sshpass", "-p", password, "ssh", fmt.Sprintf("%s@%s", username, host))
...
func main() {
	if err := ensureCommandInstalled("sshpass", "sshpass"); err != nil {
		fmt.Printf("Failed to install sshpass: %v\n", err)
		return
	}
	...
	flag.StringVar(&domain, "d", "", "Specify the domain for the scan.")
	flag.StringVar(&usernameFile, "uf", "", "Specify a file containing a list of usernames for the scan.")
	flag.StringVar(&passwordFile, "pf", "", "Specify a file containing a list of passwords for the scan.")
	flag.StringVar(&singleHash, "H", "", "Specify an NTLM hash for the scan.")
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "▐█▄▪▐█▐█▌.▐▌    ██ ██▌▐█▌▐█ ▪▐▌▐█▪·•    ▐█▄▄▌▪▐█·█▌▐█▄▄▌▐███▌\n")
		fmt.Fprintf(os.Stderr, "·▀▀▀▀  ▀█▄▀▪    ▀▀  █▪▀▀▀ ▀  ▀ .▀        ▀▀▀ •▀▀ ▀▀ ▀▀▀ ·▀▀▀\n\n")
		fmt.Fprintf(os.Stderr, "Go Map Exec: A Password Sprying tool by Tonee Marqus\n\n")
